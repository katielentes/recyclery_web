...
app.use(
  express.urlencoded({
    // ↑ if you ever post urlencoded too
    limit: '50mb',
    extended: true,
  })
);
...
app.get('/people/:id', (async (req: Request, res: Response) => {
  try {
    ...
    if (dbErr) {
      res.status(500).json({ error: 'Failed to update person record' });
      return;
    }
    if (!person) {
    ...
    res.json({
      ...
      person_description: person?.description || null,
      person_image: person?.person_image || null,
    });
  } catch (error) {
...
app.get('/hours/:id', (async (req: Request, res: Response) => {
  try {
    ...
    if (dbErr) {
      res.status(500).json({ error: 'Failed to get hours record' });
      return;
    }
    if (!hours) {
      res.status(404).json({ error: 'Hours not found' });
      return;
    }
    res.setHeader('Content-Type', 'application/json');
...
// for UploadHours
app.put('/uploadhours/:id', express.json({ limit: '1mb' }), (async (
  req,
  res,
  next: NextFunction
) => {
...
// PUT for upload WHO
app.put('/uploadpeople/:id', upload.single('file'), (async (
  req: MulterRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (req.file) {
      ...
      if (upErr) {
        res.status(500).json({ error: 'Failed to upload image to WHO bucket' });
        return;
      }
      // 4) getPublicUrl is sync
      const { data: urlData } = supabase.storage.from('who').getPublicUrl(key);
