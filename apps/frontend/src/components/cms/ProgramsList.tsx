...
import { RichTextBlock, RichTextChild, RichTextContent, cmsService } from '../../services/cms.js';
    ...
    return (
      <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
        ...
        <div className="mt-2 text-sm">
          Make sure Strapi is running and the Programs content type has public permissions.
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {programs.map(program => {
          ...
          const convertRichTextToString = (richText: RichTextContent): string => {
          ...
          return (
            ...
            >
              {/* Hero Image */}
              {program.heroImage?.data?.attributes?.url && (
                <div className="aspect-video w-full overflow-hidden">
                  <img
                    src={cmsService.getMediaUrl(program.heroImage.data.attributes.url)}
                    alt={program.heroImage.data.attributes.alternativeText || program.title}
              ...
              <div className="p-4">
                ...
                {program.shortDescription && (
                  <p className="text-gray-600 text-sm mb-3">{program.shortDescription}</p>
                )}
                ...
                {program.content && (
                  <div className="text-gray-500 text-xs mb-3">
                    <details>
                      <summary className="cursor-pointer hover:text-gray-700">Read more...</summary>
                      <div className="mt-2 text-sm whitespace-pre-line">
                        {convertRichTextToString(program.content)}
                      </div>
                    </details>
                  </div>
                )}
